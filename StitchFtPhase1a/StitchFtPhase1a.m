...
classdef StitchFtPhase1a < handle
...
% Constructor
%==================================================================   
        function [obj] = StitchFtPhase1a(Options)
            obj.Options = Options;
            obj.Log = Log('');
        end                
...
%================================================================== 
        function Process(obj)
            ...
            %--
            obj.Image(:,:,:,:,2,:) = exp(1i*angle(obj.Image(:,:,:,:,1,:)));
            obj.Image(:,:,:,:,2,:) = fftshift(fftn(ifftshift(obj.Image(:,:,:,:,2,:))));
            ImageDims = obj.Stitch{1}.ImageReturnDims;
            start = ImageDims(1)/2 - 6;
            stop = ImageDims(1)/2 + 7;
            Centre = obj.Image(start:stop,start:stop,start:stop,:,2,:);
            obj.Image(:,:,:,:,2,:) = zeros(size(obj.Image(:,:,:,:,2,:)));
            obj.Image(start:stop,start:stop,start:stop,:,2,:) = Centre;
            obj.Image(:,:,:,:,2,:) = fftshift(ifftn(ifftshift(obj.Image(:,:,:,:,2,:))));
            %--
        end
