...
function TrajMashInfo = TrajMash20RespPhasesGaussianWithMask(k0,MetaData)
...
while(1)
    ...
    remove_inds(j,:)=round(x);
    j=j+1;
end
close(hfig)
mask=ones(1,NumAcqs);
for(j=1:size(remove_inds,1))
    mask(remove_inds(j,1):remove_inds(j,2))=0;
end
