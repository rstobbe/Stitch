...
function TrajMashInfo = TrajMash20RespPhasesGaussian_SpecAverages(k0,MetaData)
...
if(isfield(MetaData,'UseAverages'))
    remove_averages=[1:MetaData.NumAverages];
    remove_averages(MetaData.UseAverages)=[];
end
for RespPhaseNum = 1:NumImages  
    ...
    for Traj = 1:MetaData.NumTraj
        ...
        if(exist('remove_averages','var'))
            Weight(remove_averages) = 0;
        end
